SELECT DISTINCT FLAN.IDLAN,
                ...
                SUBSTR (FLAN.NUMERODOCUMENTO, LENGTH(FLAN.NUMERODOCUMENTO) - 1 + 1, 1) AS R,
                ID_FATURA,
        ...
        FROM   U_CFL8U4_RM.TMOV
               ...
               LEFT JOIN U_CFL8U4_RM.GMUNICIPIO
                      ON GFILIAL.CODMUNICIPIO = GMUNICIPIO.CODMUNICIPIO
                         AND FLAN.IDFAT IS NOT NULL) TAB1
       LEFT JOIN U_CFL8U4_RM.FLAN
...
WHERE  FLAN.CODAPLICACAO = 'F'
       ...
       AND SUBSTR (FLAN.NUMERODOCUMENTO, LENGTH(FLAN.NUMERODOCUMENTO) - 1 + 1, 1) >= :PARCELA_INICIAL
       AND SUBSTR (FLAN.NUMERODOCUMENTO, LENGTH(FLAN.NUMERODOCUMENTO) - 1 + 1, 1) <= :PARCELA_FINAL
/* AND FLAN.DATAVENCIMENTO >= :VENCIMENTO_INI
AND FLAN.DATAVENCIMENTO <= :VENCIMENTO_FIM
AND FLAN.CODCOLIGADA = :CODCOLIGADA  FLAN.DATAVENCIMENTO = :VENCIMENTO
